...
import arcgisscripting, arcpy, sys, string, os, re, time
from time import *
...
try:
    ...
    if InWaterBody <> "#":
        ...
        #(in_zone_data, zone_field, in_value_raster, {statistics_type}, {ignore_nodata})
        gp.ZonalStatistics(InWaterBody, "VALUE", InReach, "burntemp","MAXIMUM", "DATA")
        InRaster = "burntemp"
    else:
        InRaster = InReach
    ...
    arcpy.env.workspace = gp.Workspace
    gp.BuildRasterAttributeTable(rca_ras, "Overwrite")
    ...
    gp.Dissolve_management("temp_rca.shp", OutRCAShapeFile + ".shp", "GRIDCODE", "#", "SINGLE_PART")
    ...
    gp.AddMessage("DID NOT Finish Create Cost RCAs Script")
    gp.GetMessages()

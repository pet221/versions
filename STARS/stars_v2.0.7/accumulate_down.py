...
# Last Modified by Erin Peterson 04/26/17

import arcgisscripting, arcpy, sys, string, os, re, time, win32com.client, win32api
from time import *
...
try:
    ...
    conn.Open(DSN)
    arcpy.env.workspace = gp.Workspace
    ...
    if tb: # IF ReltableName exists then 
        ...
        arcpy.MakeFeatureLayer_management(FeatureclassName, "edgelyr")
        arcpy.SelectLayerByAttribute_management("edgelyr", "ADD_TO_SELECTION", string)

        calcfield =  "[" + AccField + "]"
        arcpy.CalculateField_management("edgelyr", OutField, calcfield)

        arcpy.SelectLayerByAttribute_management("edgelyr", "CLEAR_SELECTION")
